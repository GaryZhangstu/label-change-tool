...
class AnnotationApp:
    ...
    def __init__(self, root):
        ...
        self.upload_button = tk.Button(
            root, text="Upload JSON File", command=self.upload_json
        )
        self.upload_button.grid(row=0, column=0, padx=10, pady=5, sticky="w")

        self.open_folder_button = tk.Button(
            root, text="Open Images Folder", command=self.open_images_folder
        )
        ...
        self.class_name_menu = tk.OptionMenu(
            root,
            self.class_name_var,
            *self.class_name_options,
            command=self.update_class_name,
        )
        self.class_name_menu.grid(row=2, column=0, padx=10, pady=5, sticky="w")

        self.previous_image_button = tk.Button(
            root, text="Previous Image", command=self.previous_image
        )
        self.previous_image_button.grid(row=4, column=0, padx=10, pady=5, sticky="w")

        self.next_image_button = tk.Button(
            root, text="Next Image", command=self.next_image
        )
    ...
    def load_image(self):
        ...
        if not self.json_data or not self.images_folder:
            messagebox.showwarning(
                "Warning", "Please load JSON file and images folder first."
            )
            return
        ...
        if os.path.exists(image_path):
            ...
            self.image_on_canvas = self.image_on_canvas = self.image_on_canvas = (
            ...
            messagebox.showwarning(
                "Warning", f"Image {image_info['file_name']} not found in folder."
            )
    ...
    def load_annotations(self, image_id, scale_x, scale_y):
        ...
        for ann in self.annotations:
            ...
            annotation_id = self.canvas.create_oval(
                x - radius,
                y - radius,
                x + radius,
                y + radius,
                outline="red",
                width=2,
                tags=("annotation", f"annotation_{ann['id']}"),
            )
    ...
    def zoom_image(self, event):
        ...
        if self.current_image_index < len(images):
            ...
            if os.path.exists(image_path):
                image = Image.open(image_path)
                new_size = (
                    int(image.width * self.zoom_scale),
                    int(image.height * self.zoom_scale),
                )
                ...
                self.load_annotations(
                    image_info["id"],
                    new_size[0] / image_info["width"],
                    new_size[1] / image_info["height"],
                )
                self.canvas.tag_raise("annotation")
